...
- ✅ O6 (Seq-Item Predicate Consolidation): **1.6-6.15x faster on bare-dash sequence items**, issue #106
  ...
  - **Attribution**: #332 landed most of the consolidation on `main` independently (named it `starts_seq_entry`, routed `uncons`/`get` through `value()`) but kept `uncons_cursor` on a narrower `starts_inline_seq_entry`, leaving the `-\n` fall-through — and the pathology — fully intact. Widening that last site is this branch's contribution
  ...
  - **Measured** (interleaved A/B, min of 15, both machines verified idle): Apple M4 Pro seqwrap/1mb 1.62x, bare-dash 1MB 2.19x, **4MB 6.12x**; AMD Ryzen 9 7950X seqwrap/1mb 2.97x, bare-dash 1MB 4.77x, **4MB 16.37x**. Speedup grows with size (the signature of removing an O(N·L) term); users/sequences/nested/comprehensive neutral (0.99-1.01x) on both
  - **Zen 4 suffered ~3x more than Apple Silicon pre-fix** (2239 ms vs 758 ms at 4MB) while post-fix times match (137 ms vs 124 ms) — the pathology is a cache-thrashing IB rescan, so an Apple-only measurement understates it by 2.7x
  - Outputs byte-identical pre↔post on 48 configurations per machine, all still matching system `yq`
