...
- ✅ O6 (Lazy `ib_rank` + Index Memory Accounting): **−10% retained index**, 38/44 `yaml_bench` improved, issue #56
  - Added `YamlIndex::memory()` and a CI-checked `## YAML index memory` section in `corpus-stats` — the index was previously **unmeasurable from production code**
  - `AdvancePositions::ib_rank` (L/16 bytes, a third of the structure) is now built lazily via `OnceCell`; its only reader is the reverse text→BP lookup behind `yq-locate` and jq `at_offset`/`at_position`
  - **Measured** (Apple M4 Pro, 1MB synthetic corpus): retained index 3,262,295 → 2,934,567 bytes (−10.0%, 62.2% → 56.0% of input); `open_positions` −30.1%; `yaml_bench` 38 significant improvements, **0 regressions**, median −3.88%; `yq` output byte-identical across 120 configurations
  - **Corrected the record**: docs claimed 3-6% index overhead; measured is ~56% of input. The `advance_positions.rs` "~3.5× smaller" claim omitted both rank arrays and assumed a density only the synthetic generators produce
  - **Key finding**: the compact position encodings cost Θ(L) *independent of node count*, so their ratio to dense `Vec<u32>` is ~`0.05·(L/M)` — unbounded. Dropping the eager `ib_rank` moved break-even from `L/M ≈ 20` to `≈ 30`, pulling the two real corpus files that had been choosing the losing encoding (`actions/prometheus-ci.yml` at `L/M = 20.7`, `actions/stale.yml` at 23.9) back onto the winning side (3752 → 2564 vs 3660 dense; 276 → 188 vs 216). Selection is still by monotonicity alone, so a one-block-scalar document still pays kilobytes for 4 nodes
  - ❌ Trimming the parser's 1.75× pre-allocation: **REJECTED** — cut allocator-counted peak 15-31% but real RSS 0-1.8%, because `vec![0u64; n]` pages are lazily zeroed and never faulted in. A tracking allocator counts requested bytes, not resident pages; this also retires issue #74's "−30% peak memory during parse" estimate
  - Fixed two latent bugs: IB bitmap sized one bit too small (null sentinel at `input.len()` lost when `L % 64 == 0`), and `binary_search` over the `Dense` variant, which is unsorted by construction (`yq-locate` silently missed nodes after a valueless explicit key)
  - See [docs/parsing/yaml.md#o6-lazy-ib_rank-and-index-memory-accounting--accepted-](docs/parsing/yaml.md#o6-lazy-ib_rank-and-index-memory-accounting--accepted-) for full analysis
