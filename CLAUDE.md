...
- A single figure can't describe two rows whose reachable code differs 3.4x in instructions executed: #2608's canonical-compact echo was closed on an A/B reading "+7% on both the canonical document and the late-fail twin" (586M vs 1,985M Ir) — a delta that reads the same whether the fast path fires or bails is measuring something other than the path (layout, a binary not taking the path, a corpus that isn't the shape). Re-measured with a `black_box(false)` holdout and a control copy: −69%/+9% wall on an M4 Pro, −72%/+3% wall and −66.6%/+13% Ir on a 7950X
- The echo is free and the decision is the cost (#2608): 32.7 Ir/byte to decide whether a compact jq render can skip re-rendering, 0.02 to copy the span once it may — and a −4.8% Ir SIMD win on that decision (a jq-table escape scanner plus #2963's word probe) lost 3-9% wall-clock on both boxes, on strings 4-12 bytes wide, below the width where a NEON/AVX compare→movemask→GPR round trip beats a short scalar loop (#2963's per-item lesson again)
