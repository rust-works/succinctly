...
fn stream_transcode_double_quoted_to_json<Out: core::fmt::Write>(
    ...
    out.write_char('"').map_err(|_| YamlStringError::InvalidUtf8)?;
    let mut i = 0;
    ...
    while i < bytes.len() {
        match bytes[i] {
            b'\\' => {
                ...
                match bytes[i] {
                    b'n' => out.write_str("\\n").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'r' => out.write_str("\\r").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b't' | b'\t' => out.write_str("\\t").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'"' => out.write_str("\\\"").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'\\' => out.write_str("\\\\").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'/' => out.write_char('/').map_err(|_| YamlStringError::InvalidUtf8)?,
                    b' ' => out.write_char(' ').map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'0' => out.write_str("\\u0000").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'a' => out.write_str("\\u0007").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'b' => out.write_str("\\u0008").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'v' => out.write_str("\\u000b").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'f' => out.write_str("\\u000c").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'e' => out.write_str("\\u001b").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'N' => out.write_str("\\u0085").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'_' => out.write_str("\\u00a0").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'L' => out.write_str("\\u2028").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'P' => out.write_str("\\u2029").map_err(|_| YamlStringError::InvalidUtf8)?,
                    b'\n' => {
                        i += 1;
                        while i < bytes.len() && matches!(bytes[i], b' ' | b'\t') {
                            i += 1;
                    ...
                    b'x' => {
                        ...
                        if val < 0x20 || val == 0x22 || val == 0x5C {
                            ...
                            out.write_char(val as u8 as char).map_err(|_| YamlStringError::InvalidUtf8)?;
                        } else {
                            stream_json_escape(out, ch).map_err(|_| YamlStringError::InvalidUtf8)?;
                        }
                        i += 2;
            ...
            b'"' => {
                out.write_str("\\\"").map_err(|_| YamlStringError::InvalidUtf8)?;
                i += 1;
            }
            b if b < 0x20 => {
            ...
            _ => {
                ...
                let chunk = core::str::from_utf8(&bytes[start..i])
                    .map_err(|_| YamlStringError::InvalidUtf8)?;
                out.write_str(chunk).map_err(|_| YamlStringError::InvalidUtf8)?;
            }
        }
    }
