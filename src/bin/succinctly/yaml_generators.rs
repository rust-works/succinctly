...
///
/// Single source of truth. `yaml generate-suite` and `dev bench yq` both derive
/// their pattern lists from this, and `tests/yaml_bench_suite_coverage.rs`
/// checks it against the `yaml generate --pattern` value list, so a pattern
/// cannot be registered in one place and forgotten in another.
///
