...
fn add_simple_values(json: &mut String, target_size: usize, rng: &mut Option<ChaCha8Rng>) {
    ...
    while json.len() - start_len < target_size {
        ...
        let val = match rng.as_mut().map_or(count % 4, |r| r.random_range(0..4)) {
            0 => "true".to_string(),
            1 => "false".to_string(),
            2 => "null".to_string(),
            _ => count.to_string(),
    ...
    while json.len() - start_len < target_size {
        ...
        let num = match count % 8 {
            ...
            _ => {
                let val = rng
                    .as_mut()
                    .map_or(count as f64, |r| r.random::<f64>() * 1000000.0 - 500000.0);
                format!("{val:.6}")
            }
        };
...
fn add_array_variations(
    ...
    while json.len() - start_len < target_size {
        ...
        first = false;
        json.push('[');
        let array_len = rng.as_mut().map_or(5, |r| r.random_range(3..10));
...
fn add_tree_structure(
    ...
    let num_children = rng.as_mut().map_or(3, |r| r.random_range(2..5));
...
fn add_realistic_records(
    ...
    while json.len() - start_len < target_size {
        ...
        let name = if rng
            .as_mut()
            .is_some_and(|r| r.random::<f64>() < escape_density)
        {
...
pub fn generate_arrays_json(target_size: usize, seed: Option<u64>) -> String {
    ...
    for i in 0..num_elements {
        ...
        for j in 0..5 {
            ...
            let val = rng.as_mut().map_or(j, |r| r.random_range(0..1000));
...
pub fn generate_mixed_json(target_size: usize, seed: Option<u64>) -> String {
    ...
    for i in 0..num_items {
        ...
        let pattern_idx = rng
            .as_mut()
            .map_or(i % patterns.len(), |r| r.random_range(0..patterns.len()));
...
pub fn generate_literals_json(target_size: usize, seed: Option<u64>) -> String {
    ...
    while json.len() < target_size.saturating_sub(10) {
        ...
        let val = match rng.as_mut().map_or(count % 3, |r| r.random_range(0..3)) {
            0 => "true",
            1 => "false",
            _ => "null",
        };
