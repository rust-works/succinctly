...
- **An 8-byte word probe** in `string_literal_end` (`STRING_SCALAR_PREFIX`, aarch64
  ...
  #3100 traced that +17% to placement. The math-builtin commit between the two mains
  moved the JSON index builder's per-byte jump-table loop
  (`json::simd::avx2::process_chunk_standard`) by 176 bytes. Inert padding that moves it
  the same way, with no source change, reproduces the slowdown, and the effect repeats
  every 64 bytes, a 15% band on this shape. See
