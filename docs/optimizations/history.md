...
**Key insight**: Allocation count and pass count were not the bottleneck at these field sizes — array iteration and output formatting dominate instead, matching #124's own finding that `@csv`/`@sh` were "already fast." A separate, unrelated fix deduplicating `@csv`'s/`@dsv`'s quoting logic was split out to [#651](https://github.com/rust-works/succinctly/issues/651) and merged independently of this (non-)performance outcome. PR #650 was closed without merging; its `dsv_records`/`sh_field` bench coverage and multi-byte regression tests were salvaged separately via [#657](https://github.com/rust-works/succinctly/issues/657)/[PR #658](https://github.com/rust-works/succinctly/pull/658).
