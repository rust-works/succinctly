...
1. **Variable-rooted navigation off a mismatched accumulator** — **closed by
   ...
   **Code review widened the fold and narrowed the peel.** `eval_owned_relocating_fold`
   ...
   | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | `{k:.} \| .k \| path($x)` on a scalar root (`"s"`, `5`), and `[.] \| path(.[0] \| $x)` on one                                                                              | scalars (`OwnedValue::String`, numbers) are not `Rc`-backed, so they never enter the embed table and have no storage for #3177's clause to share                                                                                                                                                                                                                                |
   ...
   **[#3177](https://github.com/rust-works/succinctly/issues/3177), now closed: the embed
   ...
   path($x)`) was separate: `fold_lazy_seq_stage` now hands an instruction-free cursor
   sequence through an identity stage untouched, so the fold sees the same array the wider
   spellings did — gated on "no instructions" on purpose, since with a `map(f)` on the
   sequence the `.` is a materialization barrier that keeps #725's lazy `first` divergence
   from spreading to a new spelling. What stays refused is in the table above: scalars, a
   `path()` reached only through a wrapper (#3189), the `del`/assignment resolvers (#3188),
