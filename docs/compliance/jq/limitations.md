...
1. **Variable-rooted navigation off a mismatched accumulator** — **closed by
   ...
   **Code review widened the fold and narrowed the peel.** `eval_owned_relocating_fold`
   ...
   | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | `tostring \| tostring`, a no-match `ltrimstr`/`rtrimstr`/`sub`, `abs`, `[.] \| sort\|unique\|flatten\|reverse \| .[0]`, `setpath([]; .)`, `def f: 5; f as $x \| f` (each `\| path($x)` on a scalar root `"abc"`/`5`)              | jq hands its input `jv` back from each of these; since #3182 a string or number literal shares storage like a container, but these builtins are not ones the generic evaluator answers over the cursor, so its catch-all re-indexes the input before `eval.rs`'s builtin runs and `path()` is handed a fresh copy -- the scalar twin of the `sort` row below (`scalar-*-keeps-*` in the sweep)                                                            |
