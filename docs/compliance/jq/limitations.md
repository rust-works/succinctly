...
1. **Variable-rooted navigation off a mismatched accumulator** — **closed by
   ...
   **[#2889](https://github.com/rust-works/succinctly/issues/2889) Stage B, now landed,
   ...
   | ----------------------------------------------------------------------------------- | ----------------------------------------------- | ---------- | ------------------------------------------------------------------------ |
   | `[path(([1] \| unique) \| empty)]`, and `unique_by`/`map_values`/`with_entries`, `walk` on an *object*, `sub`/`gsub` (with or without flags) | raises, naming a **derived** container (`iterate through [[1]]`) | raises the same `ErrorKind` (uncatchable in path position, ordinarily catchable in value position — same as jq), naming `value` — the resolver's own input, not jq's `to_entries`/`group_by`/`match`/`_modify` intermediate | [#3271](https://github.com/rust-works/succinctly/issues/3271), narrowing [#2743](https://github.com/rust-works/succinctly/issues/2743) |
