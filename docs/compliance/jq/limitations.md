...
1. **Variable-rooted navigation off a mismatched accumulator** — **closed by
   ...
   **[#3178](https://github.com/rust-works/succinctly/issues/3178), now closed: element identity
   through `sort`, `unique`, `reverse`, `to_entries` and a literal `getpath`.** jq builds each
   result from its input's own children, so `. as $x \| [.] \| sort \| .[0] \| path($x)` is `[]`
   there, and `{k:.} \| getpath(["k"])` hands back the node itself. `eval_owned_relocating_fold`,
   #2889's route for `add`/`min`/`max`, now runs these too, both as the whole re-entered
   expression and as a leading pipe stage (`embed_peel_step`). It runs the bridged builtins'
   own calls (`compare_values`' stable sort, `owned_value_eq`'s dedup that keeps the first of
   each equal run, one shared `owned_to_entries`) over the cloned children, so every value is
   ...
   **[#3179](https://github.com/rust-works/succinctly/issues/3179), now closed: an embed reached
   ...
   path($y)` is `[]` in jq: `{k:.}` places the root's own `jv`, whose `.a` is `$y`'s. The embed
   table was consulted only at `to_owned_cursor`'s depth 0, so materializing the root built `.a`
   afresh. The walk now hands back an entry's own value for a nested container too, after one
   precheck per materialization (is any entry's node inside this subtree at all -- a BP
   `find_close`, asked only when an entry opens after the cursor) and only when the reuse depth
