...
Two positions are deliberately **not** full expressions:
...
**Multi-output expressions in non-fanout positions** parse but take only their
first output: `range(1,2; 4)`, `"\(1,2)"`, `{a: (1,2)}`, `select(.==1, .==3)`
and a computed object key `{(("a","b")): 1}` all yield one result where jq
yields one per output. Before #462 several of these were parse errors rather
than wrong answers, since the comma could not be written without parens.
