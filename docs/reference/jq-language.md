...
A float **slice bound** is not covered by that and still diverges (#1326): `path(.[1.5:3.5])` reports `[{"start":1,"end":4}]`, the bounds as *resolved*, where jq keeps `[{"start":1.5,"end":3.5}]`, the bounds as *written*. The slice itself is unaffected — both read `[2,3,4]`.
