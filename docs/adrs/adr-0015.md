...
unit test but not load-bearing either: the CLI tool's corpus-shape report
(in `src/bin/succinctly/corpus_stats.rs`) builds a throwaway `BitVec` at
...
diagnostic printed in the report, not a use of `BitVec`'s rank/select to
serve a query.
