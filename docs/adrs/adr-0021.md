...
8. **No regrowth.** `tests/jq_path_context_arm_guard.rs` pins the eager evaluator's named
   ...
   **Exit condition met, 2026-09-07 (#2559).** The evaluator is deleted -- 2,579 lines and
   ...
   (#2572 has since removed the clones. Its own A/B, against the walk as it stood just
   before that change rather than the measurement here, is recorded in
...
emitted node must be an untagged scalar spelling exactly the member's own display key —
and every other case (an explicitly tagged key, a complex `? [a, b]` key, an undecodable
one) keeps the display string byte-for-byte. #2785 widened the guard to a *typed* key
(`1: x`, `true: y`, `null: z`): real yq's `key` is an `!!int`/`!!bool`/`!!null` node
there, and once yq-mode `==` compared scalars by text (`eval::yq_scalar_text_eq`, the
divergence #2763 had stopped at), emitting the node no longer broke `select(key == "1")`.
The type is decided where the key is materialized (`key_owned_value`, and the walk's
`path_context_item_to_owned`) behind the same spelling prefilter, and `keys`,
`to_entries` and `with_entries` take the same helper so no reader can answer a different
type from another.
...
`docs/compliance/yq/limitations.md`: a typed key's lost spelling and the string path
register (#2785's residuals; the typed key itself was #2763's residual until then), a
`key | <metadata>` nested inside a
...
- [#2763](https://github.com/rust-works/succinctly/issues/2763) — the amendment above: the
  ...
  [#2785](https://github.com/rust-works/succinctly/issues/2785) — a typed key is that node
  too, once yq-mode `==` compares scalars by text ·
