...
7. **An owned value carries its position.** Real yq's nodes carry parent pointers, and a
   ...
   **The residue is a head, not a stage.** `.[] | .k | select(key == "k")` is one position
   per element, the memory cost `path_context_fans_out` exists to refuse and the same
   guard `path_context_walk_split` applies (and `.[]?` counts, since #2558 gave that
   predicate its own `Expr::Optional` arm). It stays eager by design, and it is what
   decision 8's exit condition has to survive: this route is not what will remove it.
