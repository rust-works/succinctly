...
7. **An owned value carries its position.** Real yq's nodes carry parent pointers, and a
   ...
   **The residue is a head, not a stage.** `.[] | .k | select(key == "k")` is one position
   ...
   Beside it, after the identity pass: a fan-out or escaping *component* (`.c[(0,1)]`,
   `.c[halt]`), a slice or `getpath` head (the third `PathNode` variant), `..` (#2428), a
   builtin with no captured rule, a computed `parent(n)`, and a bare `$x` stage.
