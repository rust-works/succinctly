...
> **Update (#125 / O5):** the scanner was relocated to the shared
> [`src/util/simd/escape.rs`](../../src/util/simd/escape.rs) module so consumers
> (jq streaming, jq/yq output escaping) no longer reach into `yaml/simd`
> internals. The public `find_json_escape` API and its behaviour are unchanged
> and re-exported from `yaml::simd` for compatibility; the per-backend kernels are
> now generated by a predicate-parameterized `define_escape_scanner!` macro (so a
...
> where the hot loop lives today; see the "Where this code lives now" note
> further down in this section for the current shape.
