...
#### What the pinned profile does not pin: function placement (#3100)

`cgu1+fat` closed the lottery that came from *partitioning*. It did not close the one that
comes from *placement*, and #3100 is the fourth instance of the class (after #595, #1587
and #2603/#2655). `220cc911b` → `71569fc87` timed the 7950X's `jq keys_unsorted` on the
10 MB `users` document **+12% (min) / +18% (median)** slower. `length` was just as slow, and
every row of the `users`/`arrays`/`wide` × `keys_unsorted`/`.`/`length`/`.[]` table was
slower, against a control range of −0.9%..+1.7%. Instruction counts were identical. The
whole delta bisects to `e11cd6da8` (#2937: math-builtin widening, which is not on any of
these queries' paths). The seven test-only commits after it leave the stripped binary
...
not only `process_chunk_standard`, so pinning the cause on that one function rests on its
share of the indirect branches, not on a test that isolates it.

**No build flag fixes it cheaply.** Each LLVM alignment option was applied to both sides:

| `RUSTFLAGS` (`-C llvm-args=...`)  | `A` vs `e11cd6da8` | `A` vs `A` + flag | effect                                                   |
|-----------------------------------|--------------------|-------------------|----------------------------------------------------------|
| `-align-all-functions=6`          | +1.7%              | +11.2%            | cancels the delta by putting both sides at the slow spot |
