...
#
# The `scalar-*` rows are #3182 (ADR-0024's option D): a string or a number
# literal is `Rc`-backed like a container, so a scalar root or element bound by
# `as` keeps its identity through the same placements (`{k:.} | .k`, `[.] |
# .[0]`, `[.] | add`) -- and, captured live, through every builtin real jq hands
# its input `jv` back from (`tostring`/`@text` on a string, a no-match
# `ltrimstr`/`rtrimstr`/`sub`, `tonumber` on a number, `abs`, `getpath([])`,
# `setpath([]; .)`), while every builtin that allocates its result
# (`ascii_downcase`, `. + ""`, `tojson | fromjson`, a slice, `floor`, `. + 0`)
# refuses in both. The `scalar-constant-pool-*` rows pin jq's constant pool:
# one AST literal evaluated twice is one `jv` (`def f: 5; f as $x | f`), two
# literals are two, and two equal document scalars are two nodes. `null`/`bool`
# keep their by-value identity and a computed number (`(.+0)`) has none.
#
...
scalar-string-keeps-tostring-twice:#3182 -- the first tostring answers on the cursor (its bypass hands the string's own handle back), the second runs over that owned value through the reindex bridge and gets a fresh copy: same class as scalar-string-keeps-ltrimstr-nomatch
scalar-string-keeps-ltrimstr-nomatch:#3182 -- the builtin is not one the generic evaluator answers over the cursor, so its catch-all materializes the input and re-indexes it before eval.rs's builtin runs; the value it hands back is a fresh copy, the same bridge owned-embed-refuse-sort-element and owned-embed-refuse-path-nested-after-sort cross (#2889/#3177)
scalar-string-keeps-ltrimstr-nonstring-arg:same as scalar-string-keeps-ltrimstr-nomatch
scalar-string-keeps-rtrimstr-nomatch:same as scalar-string-keeps-ltrimstr-nomatch
scalar-string-keeps-sub-nomatch:same as scalar-string-keeps-ltrimstr-nomatch (jq's sub hands the input back when nothing matched; the bridge copies it first)
scalar-string-keeps-gsub-nomatch:same as scalar-string-keeps-sub-nomatch
scalar-string-keeps-sort-one:#3182 -- the scalar twin of owned-embed-refuse-sort-element: sort re-indexes the array before the element is read
scalar-string-keeps-unique-one:same as scalar-string-keeps-sort-one, for unique
scalar-string-keeps-flatten-one:same as scalar-string-keeps-sort-one, for flatten (jq's flatten places the element's own jv)
scalar-string-keeps-reverse-one:same as scalar-string-keeps-sort-one, for reverse
scalar-string-keeps-setpath-empty:#3182 -- setpath is a write and runs through the assignment resolver's bridge first (owned-embed-refuse-update-noop-element's class); jq's setpath([]; v) hands v back
scalar-number-keeps-abs:#3182 -- abs is a bridged builtin (jq's abs is `if . < 0 then -. else . end`, the input jv itself for a non-negative number): same class as scalar-string-keeps-ltrimstr-nomatch
scalar-number-keeps-sort-one:same as scalar-string-keeps-sort-one, for a number
scalar-number-keeps-unique-one:same as scalar-string-keeps-unique-one, for a number
scalar-number-keeps-max-by-one:same as scalar-string-keeps-sort-one, for max_by
scalar-number-keeps-setpath-empty:same as scalar-string-keeps-setpath-empty, for a number
scalar-number-keeps-ltrimstr-passthrough:same as scalar-string-keeps-ltrimstr-nomatch (a non-string input passes through ltrimstr unchanged in jq)
scalar-constant-pool-number-def:#3182 -- the literal's own handle is shared by every evaluation of the AST node (jq's constant pool), but `f | path($x)` runs the second `f` over an owned input through the reindex bridge, which hands path() a fresh copy; the two-literal twin (scalar-constant-pool-number-two-literals) refuses in both, as it must
scalar-constant-pool-string-def:same as scalar-constant-pool-number-def, for a string
REFUSE_EOF
